...
#define swap_tail						\
	...
	Float v1 = pnorm5_raw(x,Float(0.0),Float(1.0),Float(1),Float(1));
	Float v2 = pnorm5_raw(y,Float(0.0),Float(1.0),Float(1),Float(1));
	return v2 + atomic::robust_utils::R_Log1_Exp(v1 - v2);
      }else{ // Use upper tail
	Float v2 = pnorm5_raw(x,Float(0.0),Float(1.0),Float(0),Float(1));
	Float v1 = pnorm5_raw(y,Float(0.0),Float(1.0),Float(0),Float(1));
	return v2 + atomic::robust_utils::R_Log1_Exp(v1 - v2);
      }    
    };
...
Type objective_function<Type>::operator() ()
{
  ...
  if(fun == 0){ // PDE schemes
    ...
    PARAMETER(sigma);
    if(log_p < 0)
      return rsam_pnorm::log_ipnorm(x,y,mu,sigma);
    return rsam_pnorm::pnorm5(x,mu,sigma,lower_tail,log_p);
